...
class CreateAppointmentPreviews extends UseCase {
  ...
  CreateAppointmentPreviews({required this.appointments, required this.appointmentDuration, required this.selectedDay});
  @override
  Future<List<AppointmentContract>> perform() async {
    final List<AppointmentContract> result = [];
    DateTime selectedDayStart = DateTime(selectedDay.year, selectedDay.month, selectedDay.day, 8, 0);
    DateTime selectedDayEnd = DateTime(selectedDay.year, selectedDay.month, selectedDay.day, 20, 0);
    DateTime current = selectedDayStart;
    while (current.isBefore(selectedDayEnd)) {
      final DateTime end = current.add(appointmentDuration);
      final appointment = appointments.firstWhereOrNull((element) => element.fromDate.isBefore(end) && element.toDate.isAfter(current));
      if (appointment != null) {
