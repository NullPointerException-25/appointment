...
class LocalAppointmentsDatasource extends GetxService{
 ...
 static LocalAppointmentsDatasource get to => Get.find<LocalAppointmentsDatasource>();
  ...
  Future<List<AppointmentModel>> getOverlappingAppointments(AppointmentModel appointment) async {
    final box = _store.value.box<AppointmentModel>();
    final query = box.query(AppointmentModel_.fromDate.lessThanDate(appointment.toDate).and(
 ...
 Future<List<AppointmentModel>> getAppointmentsByDateRange(DateTime from, DateTime to)async {
    final box = _store.value.box<AppointmentModel>();
    final query = box.query(AppointmentModel_.fromDate.betweenDate(from, to)).build();
